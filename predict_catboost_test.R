#!/usr/bin/env Rscript
# Train CatBoost (MultiClass) on data/processed/train.csv, predict test.csv.
# Accuracy: ceiling(prediction), clipped to integer quality 3–9.

source("src/helpers.R")

if (!requireNamespace("catboost", quietly = TRUE)) {
  stop(
    "Package 'catboost' is not installed. Run: Rscript src/install_packages.R\n",
    "Or see README for CatBoost R installation."
  )
}

TEST_PATH <- "data/processed/test.csv"
ITERATIONS <- 200L
MODEL_SEED <- SEED

accuracy_ceiling <- function(actual, predicted) {
  pred_int <- as.integer(pmax(pmin(ceiling(as.numeric(predicted)), 9L), 3L))
  mean(as.integer(actual) == pred_int)
}

train_predict_catboost <- function(X_train, y_train, X_test, seed) {
  offset <- min(y_train)
  labels <- y_train - offset
  train_pool <- catboost::catboost.load_pool(
    data = as.data.frame(X_train),
    label = labels
  )
  test_pool <- catboost::catboost.load_pool(data = as.data.frame(X_test))
  params <- list(
    loss_function = "MultiClass",
    iterations = ITERATIONS,
    depth = 6,
    learning_rate = 0.1,
    verbose = 0,
    thread_count = 1,
    random_seed = as.integer(seed)
  )
  fit <- catboost::catboost.train(train_pool, params = params)
  preds <- catboost::catboost.predict(
    fit, test_pool,
    prediction_type = "Class"
  )
  as.integer(preds) + offset
}

if (!file.exists(TEST_PATH)) {
  stop("Test file not found: ", TEST_PATH)
}

train_raw <- load_train_data()
test_raw <- read.csv(TEST_PATH, sep = ";", check.names = FALSE)

if (!"quality" %in% colnames(test_raw)) {
  stop("test.csv must contain a 'quality' column to compute accuracy.")
}

proc_train <- preprocess(train_raw)
proc_test <- apply_preprocess(test_raw, proc_train$scale_params)

pred_num <- train_predict_catboost(
  proc_train$X, proc_train$y, proc_test$X, MODEL_SEED
)
y_te <- proc_test$y
acc <- accuracy_ceiling(y_te, pred_num)

cat("=== CatBoost (MultiClass) | train -> test ===\n")
cat(sprintf("Iterations: %d  random_seed: %d\n", ITERATIONS, MODEL_SEED))
cat(sprintf("Test rows: %d\n", nrow(test_raw)))
cat(sprintf("Accuracy (ceiling, clipped 3–9): %.4f\n", acc))

output_dir <- "outputs/results"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

out_df <- data.frame(
  actual_quality = y_te,
  pred_catboost_class = pred_num,
  pred_catboost_ceiling = as.integer(pmax(pmin(ceiling(pred_num), 9L), 3L)),
  stringsAsFactors = FALSE
)
write.csv(out_df, file.path(output_dir, "catboost_test_predictions.csv"), row.names = FALSE)
write.csv(
  data.frame(
    model = sprintf("CatBoost MultiClass (%d iterations)", ITERATIONS),
    accuracy_ceiling_clipped = acc,
    stringsAsFactors = FALSE
  ),
  file.path(output_dir, "catboost_test_accuracy.csv"),
  row.names = FALSE
)

cat("\nWrote:\n  ", file.path(output_dir, "catboost_test_predictions.csv"), "\n", sep = "")
cat("  ", file.path(output_dir, "catboost_test_accuracy.csv"), "\n", sep = "")
